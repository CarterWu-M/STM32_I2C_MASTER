...
int I2CSlave_task(void)
{
	...
	if (isRxDone) {
		...
		if (Cmd_unpack(&addr, &cmdId, arrSlRx, rxCnt, arrReq, &reqLen)) {
			return -1;
		}

		ret = Cmd_procS(cmdId, arrReq, &reqLen, arrRes, &resLen);

		if (resLen) {
			// for the read command response
			if (Cmd_pack(addr, cmdId, arrRes, resLen, arrSlTx, &txCnt)) {
				return -1;
			}
		}
		else {
			// for the write command response
			if (Cmd_pack(addr, cmdId, (uint8_t*)&ret, 1, arrSlTx, &txCnt)) {
				return -1;
			}
		}
